...
@pytest.fixture
def widget(qtbot, mock_user_manager):
    """Create an instance of the widget."""
    widget = AccountTab(mock_user_manager)
...
def test_handle_login_success(widget, mock_user_manager):
    """Test a successful login."""
    widget.login_user.setText("testuser")
    widget.login_pass.setText("password")
    widget.handle_login()
    mock_user_manager.login.assert_called_once_with("testuser", "password")
    assert widget.stacked.currentWidget() == widget.logged_in_widget
    assert "Welcome back, testuser!" in widget.welcome_label.text()

def test_handle_login_failure(widget, mock_user_manager):
    """Test a failed login."""
    mock_user_manager.login.return_value = (False, "Invalid password")
    widget.login_user.setText("testuser")
...
def test_handle_signup_success(widget, mock_user_manager):
    ...
    mock_user_manager.signup.assert_called_once_with("newuser", "newpassword", "personal")
    assert widget.stacked.currentWidget() == widget.login_widget
    assert "Account created!" in widget.status_label.text()

def test_handle_signup_password_mismatch(widget, mock_user_manager):
    """Test a signup with mismatched passwords."""
    widget.signup_user.setText("newuser")
    widget.signup_pass.setText("newpassword")
    widget.signup_confirm.setText("mismatch")
    widget.handle_signup()
    assert mock_user_manager.signup.call_count == 0
